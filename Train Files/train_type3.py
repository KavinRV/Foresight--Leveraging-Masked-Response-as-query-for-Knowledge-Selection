...
tokenized_wow = DatasetDict.load_from_disk("wow_rank_kel_base")
...
#         self.rank_head = nn.Linear(config.d_model, config.vocab_size, bias=False)
        ...
        self.rank_head = nn.Linear(config.d_model, 1, bias=False)
...
#         out.rank_score = rank_score[:, self.rank_id].view(-1, self.n_pass)
        out.rank_score = rank_score.view(-1, self.n_pass)
#         except RuntimeError:
...
# t = 2
model_dir = f"wow_rank_{mod_ckp}_type3_tem:T{model.config.temp1}S{model.config.temp2}"
